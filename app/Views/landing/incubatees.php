        ...
        <div class="rounded-lg reveal reveal-d1 border border-sky/30 bg-sky/5 hover:bg-sky/10 overflow-hidden transition-colors duration-300">
            <div class="grid grid-cols-1 md:grid-cols-[1fr_1.2fr]">
                ...
                <div class="relative min-h-[280px] md:min-h-[400px] flex items-center justify-center bg-[#0a1628] overflow-hidden">
                    <?php $inc = $incubatee ?? null; ?>
                    <?php if ($inc && !empty($inc['logoPath'])): ?>
                        <img src="<?= base_url($inc['logoPath']) ?>"
                             alt="<?= esc($inc['companyName'] ?? 'Incubatee') ?>"
                             class="absolute inset-0 w-full h-full object-contain p-10 md:p-14 z-[1]">
                    ...
                    <?php else: ?>
                        ...
                        <div class="relative z-[1] text-center">
                            <div
                                ...
                                <span
                                    class="font-display text-[1.8rem] md:text-[2.2rem] leading-none text-gold/70 select-none"><?= esc(mb_substr($inc['companyName'] ?? 'S', 0, 1)) ?></span>
                            </div>
                            <span class="font-display text-lg md:text-[1.2rem] text-white/50"><?= esc($inc['companyName'] ?? 'Startup Name') ?></span>
                        </div>
                    <?php endif; ?>
                    <span
                    ...
                    <span
                        class="absolute bottom-4 left-4 md:bottom-5 md:left-5 text-[.5rem] font-semibold tracking-[.12em] uppercase text-white/20 z-[2]"><?= $inc ? 'Cohort ' . esc($inc['cohort'] ?? '') : 'Cohort 1 · 2024' ?></span>
                </div>

                <!-- Right content -->
                <div
                    class="p-6 md:p-11 flex flex-col justify-center border-t md:border-t-0 md:border-l border-white/[.05]">
                    <h3 class="font-display text-xl md:text-[2rem] leading-[1.1] text-off mb-4"><?= esc($inc['companyName'] ?? 'Startup Name') ?></h3>
                    <p class="text-[.82rem] md:text-[.85rem] font-light leading-[1.85] text-white/40 mb-7"><?= esc($inc['shortDescription'] ?? 'An innovative startup incubated at ASOG-TBI, working on cutting-edge solutions to real-world problems.') ?></p>
                    <?php if ($inc && !empty($inc['founderName'])): ?>
                    <p class="text-[.6rem] font-semibold tracking-[.13em] uppercase text-sky/60 mb-5">Founded by <?= esc($inc['founderName']) ?></p>
                    <?php endif; ?>
                    <a href="<?= $inc ? site_url('incubatees/' . esc($inc['slug'] ?? '')) : site_url('incubatees') ?>"
