...
<section id="hero" class="relative w-full h-screen min-h-[680px] overflow-hidden">
    ...
    <div class="slide <?= $i === 0 ? 'active' : '' ?>"
        style="background-image:url('<?= base_url(esc($s['imagePath'])) ?>');background-size:cover;background-position:center top;">
    </div>
    <?php endforeach; ?>
    <?php else: ?>
    ...
    <div class="absolute bottom-0 left-0 z-[3] px-8 md:px-14 lg:px-20 pb-8 md:pb-12 w-full max-w-[960px]">
        ...
        <!-- ── Headline stack ── -->
        <div class="relative min-h-[40px] md:min-h-[90px] mb-4">
            <?php if ($hasSlides): ?>
            <?php foreach ($heroSlides as $i => $s): ?>
            <h1
