<?php
    ...
    $formUrl = $isEdit ? site_url('admin/facilities/' . $facility['id']) : site_url('admin/facilities');
?>
...
<a href="<?= site_url('admin/facilities') ?>" style="display:inline-flex;align-items:center;gap:.35rem;font-size:.75rem;color:#64748b;margin-bottom:1rem;text-decoration:none;transition:color .2s" onmouseover="this.style.color='#03558C'" onmouseout="this.style.color='#64748b'">← Back to facilities</a>
...
<div class="form-card">
    <form method="POST" action="<?= $formUrl ?>" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            <div class="form-group form-full">
                ...
                <input type="text" id="name" name="name"
                       value="<?= esc($facility['name'] ?? old('name', '')) ?>"
            ...
            <div class="form-group">
                ...
                <input type="number" id="sortOrder" name="sortOrder"
                       value="<?= esc($facility['sortOrder'] ?? old('sortOrder', '0')) ?>" min="0">
            </div>
            ...
            <div class="form-group form-full">
                ...
                <input type="text" id="shortDescription" name="shortDescription"
                       value="<?= esc($facility['shortDescription'] ?? old('shortDescription', '')) ?>"
                       placeholder="Brief preview shown on cards">
            </div>

            <div class="form-group form-full">
                <label>Content</label>
                <input type="hidden" name="content" class="quill-content"
                       value="<?= esc($facility['content'] ?? old('content', '')) ?>">
                <div class="quill-editor"></div>
            </div>

            <div class="form-group form-full">
                <label>Photo</label>
                <?php if ($isEdit && ! empty($facility['imagePath'])): ?>
                    <img src="<?= base_url($facility['imagePath']) ?>" alt="Current" class="img-preview show" id="currentImg"/>
                <?php endif; ?>
                ...
                <div class="toggle-row">
                    <label class="toggle-label">
                        <input type="checkbox" name="isPublished" value="1"
                            <?= (isset($facility) && $facility['isPublished']) || old('isPublished') ? 'checked' : '' ?>>
                        Publish
                    </label>
                </div>
            </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-gold"><?= $isEdit ? 'Update Facility' : 'Create Facility' ?></button>
            <a href="<?= site_url('admin/facilities') ?>" class="btn btn-outline">Cancel</a>
        </div>
    </form>
</div>
