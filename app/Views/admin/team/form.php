<?php
    $isEdit  = isset($member);
    $formUrl = $isEdit ? site_url('admin/team/' . $member['id']) : site_url('admin/team');
?>
...
<style>
    ...
    .img-preview{margin-top:.5rem;width:120px;height:120px;object-fit:cover;border-radius:50%;border:1px solid #e5e7eb;display:none}
    ...
    .quill-editor{min-height:180px;background:#fff;border:1px solid #d4d0cb;border-top:none;border-radius:0 0 .3rem .3rem;font-family:'DM Sans',sans-serif;font-size:.88rem}
    .quill-editor .ql-editor{min-height:180px}
</style>

<a href="<?= site_url('admin/team') ?>" style="display:inline-flex;align-items:center;gap:.35rem;font-size:.75rem;color:#64748b;margin-bottom:1rem;text-decoration:none;transition:color .2s" onmouseover="this.style.color='#03558C'" onmouseout="this.style.color='#64748b'">← Back to team</a>
...
<div class="form-card">
    <form method="POST" action="<?= $formUrl ?>" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="fullName" name="fullName"
                       value="<?= esc($member['fullName'] ?? old('fullName', '')) ?>"
            ...
            <div class="form-group">
                ...
                <input type="text" id="position" name="position"
                       value="<?= esc($member['position'] ?? old('position', '')) ?>"
                       placeholder="e.g. Director, Tech Lead" required>
            </div>

            <div class="form-group">
                <label for="email">Email</label>
                <input type="email" id="email" name="email"
                       value="<?= esc($member['email'] ?? old('email', '')) ?>"
            ...
            <div class="form-group">
                ...
                <input type="text" id="linkedinUrl" name="linkedinUrl"
                       value="<?= esc($member['linkedinUrl'] ?? old('linkedinUrl', '')) ?>"
            ...
            <div class="form-group">
                ...
                <input type="number" id="sortOrder" name="sortOrder"
                       value="<?= esc($member['sortOrder'] ?? old('sortOrder', '0')) ?>" min="0">
            </div>
            ...
            <div class="form-group form-full">
                ...
                <input type="text" id="shortBio" name="shortBio"
                       value="<?= esc($member['shortBio'] ?? old('shortBio', '')) ?>"
                       placeholder="Brief bio for card display">
            </div>

            <div class="form-group form-full">
                <label>Full Bio</label>
                <input type="hidden" name="content" class="quill-content"
                       value="<?= esc($member['content'] ?? old('content', '')) ?>">
                <div class="quill-editor"></div>
            </div>

            <div class="form-group form-full">
                <label>Photo</label>
                <?php if ($isEdit && ! empty($member['photoPath'])): ?>
                    <img src="<?= base_url($member['photoPath']) ?>" alt="Current" class="img-preview show" id="currentImg"/>
                <?php endif; ?>
                ...
                <div class="toggle-row">
                    <label class="toggle-label">
                        <input type="checkbox" name="isPublished" value="1"
                            <?= (isset($member) && $member['isPublished']) || old('isPublished') ? 'checked' : '' ?>>
                        Publish
                    </label>
                </div>
            </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-gold"><?= $isEdit ? 'Update Member' : 'Add Member' ?></button>
            <a href="<?= site_url('admin/team') ?>" class="btn btn-outline">Cancel</a>
        </div>
    </form>
</div>
