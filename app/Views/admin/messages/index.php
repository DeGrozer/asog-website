...
<script>
var BASE       = '<?= site_url('admin/messages') ?>';
var currentMsg = null;   // full message object
...
function openMsg(id) {
    ...
    fetch(BASE + '/' + id)
        ...
        .then(function(d){
            ...
            // Switch view: hide inbox, show reader
...
function doDelete() {
    ...
    .then(function(d){
        if (!d.success) return;
        // Remove the row from the inbox list
        var row = document.querySelector('.msg-row[data-id="' + id + '"]');
        if (row) row.remove();
        // Go back to inbox
        backToInbox();
...
document.addEventListener('keydown', function(e) {
    // Escape → back to inbox or close confirm
    if (e.key === 'Escape') {
    ...
    // e → archive/mark read (Gmail shortcut)
