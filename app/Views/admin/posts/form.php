<?php
    $isEdit   = isset($post);
    $formUrl  = $isEdit ? site_url('admin/posts/' . $post['id']) : site_url('admin/posts');
    $heading  = $isEdit ? 'Edit Post' : 'New Post';
?>
...
<a href="<?= site_url('admin/posts') ?>" style="display:inline-flex;align-items:center;gap:.35rem;font-size:.75rem;color:#64748b;margin-bottom:1rem;text-decoration:none;transition:color .2s" onmouseover="this.style.color='#03558C'" onmouseout="this.style.color='#64748b'">← Back to posts</a>
...
<div class="form-card">
    <form method="POST" action="<?= $formUrl ?>" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <select id="category" name="category">
                    <?php
                        $cats = ['news' => 'News', 'events' => 'Events', 'features' => 'Features & Opinion'];
                        $current = $post['category'] ?? old('category', 'news');
            ...
            <div class="form-group">
                ...
                <input type="text" id="authorName" name="authorName"
                       value="<?= esc($post['authorName'] ?? old('authorName', 'ASOG TBI')) ?>"
            ...
            <div class="form-group form-full">
                ...
                <input type="text" id="shortDescription" name="shortDescription"
                       value="<?= esc($post['shortDescription'] ?? old('shortDescription', '')) ?>"
                       placeholder="A brief preview shown on cards (max 500 chars)">
            ...
            <div class="form-group form-full">
                ...
                <?php if ($isEdit && ! empty($post['imagePath'])): ?>
                    <img src="<?= site_url($post['imagePath']) ?>" alt="Current image" class="img-preview show" id="currentImg"/>
                <?php endif; ?>
                ...
                <div class="toggle-row">
                    <label class="toggle-label">
                        <input type="checkbox" name="isPublished" value="1"
                            <?= (isset($post) && $post['isPublished']) || old('isPublished') ? 'checked' : '' ?>>
                        Publish
                    </label>
                    <label class="toggle-label">
                        <input type="checkbox" name="isFeatured" value="1"
                            <?= (isset($post) && $post['isFeatured']) || old('isFeatured') ? 'checked' : '' ?>>
                        Feature on hero
                    </label>
                </div>
            </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-gold"><?= $isEdit ? 'Update Post' : 'Create Post' ?></button>
            <a href="<?= site_url('admin/posts') ?>" class="btn btn-outline">Cancel</a>
        </div>
    </form>
</div>
