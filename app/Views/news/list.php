                ...
                <div class="grid grid-cols-1 lg:grid-cols-[1.15fr_1fr]">
                    ...
                    <div class="h-[240px] lg:h-full min-h-[280px] bg-[#e9e6e1] overflow-hidden">
                        ...
                        <div class="w-full h-full flex items-center justify-center">
                            <span class="text-[.6rem] font-semibold tracking-[.2em] uppercase text-dark/15">Cover
                                Image</span>
                        </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="p-7 md:p-10 lg:p-12 flex flex-col justify-center">
                        <div class="flex items-center gap-3 mb-4">
                            <span
                                class="text-[.52rem] font-semibold tracking-[.16em] uppercase text-gold"><?= esc(ucfirst($latestPost['category'])) ?></span>
                            <?php if ($latestPost['publishedAt']): ?>
                            <span class="text-[.52rem] text-dark/20">·</span>
                            <span
                                class="text-[.52rem] font-medium tracking-[.06em] text-dark/30"><?= date('F j, Y', strtotime($latestPost['publishedAt'])) ?></span>
                            <?php endif; ?>
                        </div>
                        <h2
                            class="font-display text-[1.4rem] md:text-[1.7rem] lg:text-[2rem] leading-[1.15] text-dark mb-4 transition-colors duration-200 group-hover:text-navy">
                            <?= esc($latestPost['title']) ?></h2>
                        <?php if (! empty($latestPost['shortDescription'])): ?>
                        <p class="text-[.88rem] font-light leading-[1.8] text-dark/45 mb-6 text-justify">
                            <?= html_entity_decode(esc(character_limiter($latestPost['shortDescription'], 200))) ?></p>
                        <?php endif; ?>
                        <?php if (! empty($latestPost['authorName'])): ?>
                        <span class="text-[.7rem] font-medium text-dark/30 mb-5">By
                ...
                <a href="<?= site_url('news/' . $post['slug']) ?>"
                    ...
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-2">
                            <span
                                class="text-[.48rem] font-semibold tracking-[.14em] uppercase text-gold"><?= esc(ucfirst($post['category'])) ?></span>
                            <?php if ($post['publishedAt']): ?>
                            <span class="text-[.48rem] text-dark/15">·</span>
                            <span
                                class="text-[.48rem] font-medium text-dark/25"><?= date('M j, Y', strtotime($post['publishedAt'])) ?></span>
                            <?php endif; ?>
                        </div>
                        <h3
                            class="font-display text-[.98rem] md:text-[1.02rem] text-dark leading-snug mb-2 transition-colors duration-200 group-hover:text-gold">
                            <?= esc($post['title']) ?></h3>
                        <?php if (! empty($post['shortDescription'])): ?>
                        <p class="text-[.76rem] font-light leading-[1.7] text-dark/40 text-justify">
                            <?= html_entity_decode(esc(character_limiter($post['shortDescription'], 90))) ?></p>
                        <?php endif; ?>
                    </div>
                </a>
                <?php endforeach; ?>
