<!-- ╔══════════════════════════════════════════════════════════════════════╗
     ║  INCUBATEES — TOC sidebar + listing                                ║
     ╚══════════════════════════════════════════════════════════════════════╝ -->
    ...
    <div class="max-w-[1200px] mx-auto relative z-[2]">
        <div class="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-8 lg:gap-12">
            ...
            <?php
                $tocTitle = 'Incubatees';
                $tocItems = [
                    ['id' => 'cohort-1', 'label' => 'Cohort 1'],
                ];
                // Dynamically add incubatee names if available
                if (! empty($incubatees)) {
                    $children = [];
                    foreach ($incubatees as $inc) {
                        $children[] = ['id' => 'incubatee-' . ($inc['slug'] ?? ''), 'label' => $inc['companyName'] ?? ''];
                    }
            ...
            <!-- ── Main Content ── -->
            <div class="min-w-0">
                <div id="cohort-1" class="scroll-mt-28"></div>

                <?php if (! empty($incubatees)): ?>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    <?php foreach ($incubatees as $incubatee): ?>
                    <div id="incubatee-<?= esc($incubatee['slug'] ?? '') ?>" class="scroll-mt-28">
                        <a href="<?= site_url('incubatees/' . $incubatee['slug']) ?>"
                           class="group no-underline block rounded-lg border border-dark/[.06] bg-white shadow-sm shadow-dark/[.04] overflow-hidden transition-all duration-300 hover:shadow-md hover:shadow-dark/[.08] hover:-translate-y-1">
                            ...
                            <div class="p-5 md:p-6">
                                ...
                                <?php endif; ?>
                                <p class="text-[.8rem] font-light leading-[1.7] text-dark/40 text-justify"><?= esc(character_limiter($incubatee['shortDescription'] ?? '', 90)) ?></p>
                            </div>
                        </a>
                    </div>
                    <?php endforeach; ?>
